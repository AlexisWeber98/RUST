...
pub fn result_exercise() {
    if read_file_contents(PathBuf::from("src/result_exercise.rs")).is_ok() {
        println!("File contents read successfully");
    }
    if read_file_contents(PathBuf::from("non-exist-file.txt")).is_err() {
        println!("File contents read failed");
    }
}
